...
function App() {
  ...
  const refreshAnalytics = async () => {
    if (!token) return;
    const [summaryResponse, trendResponse, productsResponse, warehousesResponse] = await Promise.all([
  ...
  useEffect(() => { refreshAnalytics(); }, [token]);
